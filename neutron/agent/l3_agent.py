...
FIP_NS_PREFIX = 'fip-'
SNAT_NS_PREFIX = 'snat-'
FIP_2_RTR_DEV_PREFIX = 'fpr-'
RTR_2_FIP_DEV_PREFIX = 'rfp-'
...
class L3PluginApi(n_rpc.RpcProxy):
    ...
    def get_agent_gateway_port(self, context, fip_net):
        """Get or create a agent_gateway_port."""
        return self.call(context,
...
class RouterInfo(object):
    ...
    def __init__(self, router_id, root_helper, use_namespaces, router):
        ...
        # TODO(mrsmith): DVR merge cleanup
...
class L3NATAgent(firewall_l3_agent.FWaaSL3AgentRpcCallback, manager.Manager):
    ...
    OPTS = [
        cfg.StrOpt('agent_mode', default='legacy',
                   help=_("Allowed modes are: 'legacy', 'dvr', 'dvr_snat'. "
                          ...
                          "'dvr_snat' for L3 agents running on dedicated "
                          "network nodes")),
    ...
    def __init__(self, host, conf=None):
        ...
        # TODO(mrsmith): remove once agent restart with
        # stale namespaces is supported for DVR
        root_ip = ip_lib.IPWrapper(self.root_helper)
        host_namespaces = root_ip.get_namespaces(self.root_helper)
        for ns in host_namespaces:
            try:
                if ns.startswith(FIP_NS_PREFIX):
                    self._destroy_fip_namespace(ns)
                elif ns.startswith(SNAT_NS_PREFIX):
                    self._destroy_snat_namespace(ns)
            except RuntimeError:
                LOG.exception(_('Failed to destroy stale namespace '
                                '%s'), ns)
        ...
        self.local_ips = set(xrange(2, 251))
        self.fip_priorities = set(xrange(FIP_PR_ST, FIP_PR_END))
    ...
    def _cleanup_namespaces(self, routers):
        ...
        try:
            ...
            # TODO(mrsmith): include DVR SNAT namespaces, FIP namespaces
    ...
    def _destroy_snat_namespace(self, ns_name):
        ns_ip = ip_lib.IPWrapper(self.root_helper, namespace=ns_name)
        # delete internal interfaces
        for d in ns_ip.get_devices(exclude_loopback=True):
            if d.name.startswith(SNAT_INT_DEV_PREFIX):
                LOG.debug('DVR: unplug: %s', d.name)
                self.driver.unplug(d.name, namespace=ns_name,
                                   prefix=SNAT_INT_DEV_PREFIX)

        # TODO(mrsmith): delete ext-gw-port
        LOG.debug('DVR: destroy snat ns: %s', ns_name)
        ns_ip.netns.delete(ns_name)

    def _destroy_fip_namespace(self, ns_name):
        ns_ip = ip_lib.IPWrapper(self.root_helper, namespace=ns_name)
        for d in ns_ip.get_devices(exclude_loopback=True):
            if d.name.startswith(FIP_2_RTR_DEV_PREFIX):
        ...
        LOG.debug('DVR: destroy fip ns: %s', ns_name)
    ...
    def _set_subnet_arp_info(self, ri, port):
        """Get ARP info from Plugin for existing ports for subnet."""
    ...
    def _arping(self, ns_name, interface_name, ip_address, dist=None):
        if dist:
            device = ip_lib.IPDevice(interface_name, self.root_helper,
                                     namespace=ns_name)
    ...
    def get_internal_port(self, ri, subnet_id):
        """Returns internal router port based on subnet_id."""
    ...
    def get_rtr_int_device_name(self, router_id):
        return (RTR_2_FIP_DEV_PREFIX + router_id)[:self.driver.DEV_NAME_LEN]

    def get_fip_int_device_name(self, router_id):
        return (FIP_2_RTR_DEV_PREFIX + router_id)[:self.driver.DEV_NAME_LEN]
    ...
    def get_snat_ns_name(self, ext_gw_port_id):
        return (SNAT_NS_PREFIX + ext_gw_port_id)

    def get_snat_interfaces(self, ri):
        return ri.router.get(l3_constants.SNAT_ROUTER_INTF_KEY, [])

    def get_floating_ips(self, ri):
        """Filters Floating IPs for DVR to be hosted on this agent."""
        floating_ips = ri.router.get(l3_constants.FLOATINGIP_KEY, [])
        if ri.router['distributed']:
            floating_ips = [i for i in floating_ips if i['host'] == self.host]
        return floating_ips
    ...
    def _map_internal_interfaces(self, ri, int_port, snat_ports):
        """Returns the SNAT port for the given internal interface port."""
    ...
    def agent_gateway_added(self, ns_name, ex_gw_port,
                            interface_name):
        """Adds Floating IP gateway port to FIP namespace."""
        if not ip_lib.device_exists(interface_name,
                                    root_helper=self.root_helper,
                                    namespace=ns_name):
            self.driver.plug(ex_gw_port['network_id'],
    ...
    def external_gateway_removed(self, ri, ex_gw_port,
        ...
        if ri.router['distributed']:
            ...
            if self.conf.agent_mode == 'dvr_snat' and (
                ...
                ns_name = self.get_snat_ns_name(ex_gw_port['id'])
            else:
    ...
    def _gen_snat_idx(self, cidr):
        """Generate index based on cidr for SNAT entries."""
        ip = cidr.split('/')[0]
        ip_str = ip.split('.')
        ip_num = (((int(ip_str[0])) << 24) + ((int(ip_str[1])) << 16)
                  + ((int(ip_str[2])) << 8) + (int(ip_str[3])))
        return ip_num

    def _snat_redirect_add(self, ri, gateway, sn_port, sn_int):
        """Adds rules and routes for SNAT redirection."""
        try:
            snat_idx = self._gen_snat_idx(sn_port['ip_cidr'])
    ...
    def create_rtr_2_fip_link(self, ri, network_id):
        """Creates interface between router and Floating IP namespace."""
    ...
    def floating_ip_added_dist(self, ri, fip):
        """Adds floating IP to FIP namespace."""
        floating_ip = fip['floating_ip_address']
        fixed_ip = fip['fixed_ip_address']
        rule_pr = self.fip_priorities.pop()
        ri.floating_ips_dict[floating_ip] = rule_pr
        fip_2_rtr_name = self.get_fip_int_device_name(ri.router_id)
        ipRule = ip_lib.IpRule(self.root_helper, namespace=ri.ns_name)
        ipRule.add_rule_from(fixed_ip, FIP_RT_TBL, rule_pr)
    ...
    def floating_ip_removed_dist(self, ri, fip_cidr):
        """Removes floating IP from FIP namespace."""
    ...
    def add_arp_entry(self, context, payload):
        """Adds arp entry into router namespace from RPC."""
    ...
    def delete_arp_entry(self, context, payload):
        """Deletes arp entry into router namespace from RPC."""
        arp_table = payload['arp_table']
        router_id = payload['router_id']
        ip = arp_table['ip_address']
        mac = arp_table['mac_address']
