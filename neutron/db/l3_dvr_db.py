...
class L3_NAT_with_dvr_db_mixin(l3_db.L3_NAT_db_mixin,
    ...
    def _validate_router_migration(self, router_db, router_res):
        ...
        if (router_db.extra_attributes.distributed and
            ...
            # NOTE(armando-migliaccio): distributed->centralized not supported
            LOG.info(_("Centralizing distributed router %s "
                       "is not supported"), router_db['id'])
            raise NotImplementedError()
        else:
            # TODO(carl_baldwin): revise centralized->distributed migration
            LOG.info(_("Distributing centralized router: %s"), router_db['id'])
    ...
    def add_router_interface(self, context, router_id, interface_info):
        ...
        if router.extra_attributes.distributed and router.gw_port:
            self.add_csnat_router_interface_ports(
                context, router_id, port['network_id'],
                {'subnet_id': port['fixed_ips'][0]['subnet_id']})

        return self.notify_router_interface_action(
            context, router_id, port['tenant_id'], port['id'],
            port['fixed_ips'][0]['subnet_id'], 'add_router_interface')
    ...
    def remove_router_interface(self, context, router_id, interface_info):
        ...
        return self.notify_router_interface_action(
            context, router_id, port['tenant_id'], port['id'],
            subnet['id'], 'remove_router_interface')
    ...
    def get_agent_gw_ports_exist_for_network(
        ...
        for p in ports:
            if(p['device_id'] == agent_id and
               self.get_vm_port_hostid(context, p['id'], p) == host):
                return p
